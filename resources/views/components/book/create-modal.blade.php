<x-modal name="create-book">
    <div
        ...
        x-on:open-modal.window="
            const inputs = document.querySelectorAll('#create-book-form input');
            const textareas = document.querySelectorAll('#create-book-form textarea');
            inputs.forEach(input => input.value = '');
            textareas.forEach(textare => textare.value = '');
            reset();
        "
    >
        <form id="create-book-form" action="{{ route('books.store') }}" method="POST" class="p-6 space-y-4">
            <h2 class="text-lg font-semibold">Create Book</h2>
            ...
            <div>
                ...
                <x-text-input id="create-title" name="title" class="mt-1 w-full" required />
            </div>
            <div>
                <x-input-label for="create-description" value="Description" />
                <x-textarea id="create-description" name="description" class="mt-1 w-full" rows="4" required>{{ old('description', '') }}</x-textarea>
            </div>
            <div>
                <x-input-label for="create-release-date" value="Released" />
                <x-text-input type="number" id="create-release-date" name="release_date" class="mt-1 w-full" min="1000" max="{{ now()->year }}" />
            </div>
            <div
                x-data="{
                    open: false,
            ...
            <div>
                <label for="create-image" class="block cursor-pointer py-4 text-center bg-gray-700 rounded-md">Upload Image</label>
                <input
                    type="file"
                    id="create-image"
                    name="image"
