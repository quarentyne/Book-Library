<header class="py-3 px-5 grid grid-cols-3 gap-5 border-b">
    ...
    @auth
        <div
            class="justify-self-end relative gap-2 flex"
            x-data="{ open: false }"
        >
            <button
            ...
            <x-sidebar-link href="{{ route('auth.logout') }}"
                            onclick="event.preventDefault(); document.getElementById('logout-form').submit();"
            >Log Out</x-sidebar-link>
        </div>
    ...
    @guest
        <div class="justify-self-end flex gap-2">
            <x-sidebar-link href="{{ route('auth.login') }}">Log In</x-sidebar-link>
            <x-sidebar-link href="{{ route('auth.register') }}">Sign In</x-sidebar-link>
        </div>
    @endguest
</header>
